...
/*		if ( material.attributes ) {
	...
	function renderShadowMap( scene, camera ) {
		...
		if ( ! _cameraLight ) {
			...
			_cameraLight = new THREE.Camera( _this.shadowCameraFov, 1, _this.shadowCameraNear, _this.shadowCameraFar );
		...
		for ( i = 0, il = lights.length; i < il; i ++ ) {
			...
			if ( light instanceof THREE.SpotLight && light.castShadow ) {
				...
				shadowMatrix = _shadowMatrix[ j ];
                                ...
                                _cameraLight.aspect = _this.shadowMapWidth / _this.shadowMapHeight;
